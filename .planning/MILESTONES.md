...
## v1.2 Мобильная адаптация, глобус оригинала и производительность (Shipped: 2026-09-06)

**Phases completed:** 4 phases, 8 plans, 23 tasks

**Delivered:** три правки пользователя после приёмки v1.1 опубликованы: https://thevladoss.github.io/esd-onevoice27/ (сборка 827785c, деплой 34046791832, 52 тестовых файла / 564 теста, приёмка Playwright против оригинала в docs/qa/SMOKE.md, раздел «Фаза 17 / v1.2»). Глобус первого экрана: видео оригинала с его стилями и портом canvas-частиц; огоньки карты на canvas-оверлее; цели касания на телефоне не ниже 44px.

**Stats:** 32 коммита за 2026-09-06 14:01 → 20:31 MSK, 33 файла в `src/` (+4029/−1252 строк), бандл 407,5 КБ + vendor-map 182,7 КБ + CSS 75,3 КБ, видео 1,9 МБ webm + 2,9 МБ mp4. fps на проде: hero/карта/форма 120 на 1440×900 (было 65/66/69) и 120 при CPU×4 на 390×844 (было 30/30/30); узлов SVG 282 вместо 3109. Фазы 14–16 исполнялись параллельно в worktree, фаза 17 — интеграция и приёмка.

**Known deferred items at close:** 0 open artifacts (audit-open чист); техдолг advisory-уровня (no-op селекторы огоньков в global.css, пары файлов с регистром и импорт с явным `.tsx`, нет единого E2E-теста «форма → огонёк», бандл +8,2 КБ, кнопки фильтров карты 40px на десктопе) — в milestones/v1.2-MILESTONE-AUDIT.md.

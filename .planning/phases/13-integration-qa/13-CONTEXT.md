...
- Известные принятые отклонения (записать в SMOKE): статичный ореол при reduced motion opacity .22 (глобальная политика), город необязателен, панель карточки новости `inset: auto 4px 4px`, Onest вместо Figtree.

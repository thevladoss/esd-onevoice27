...
- Пустые секции: `App.tsx` рендерит `Header`, `main` с восемью `<section id>` через `Section` с заголовком-заглушкой (например «Карта дивизиона — скоро») и `min-height: 40vh`, `Footer`. Заглушки заменяют фазы 2–4.

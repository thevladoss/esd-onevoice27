...
- Smoke фазы 1 (docs/qa/SMOKE-phase1.md): глобус из частиц в hero почти не виден на живом сайте, читаются только орбитальные дуги. В фазе 5 поднять яркость сферы: alpha точек передней стороны ≥ .85, размер точек 1.4–2.6px, усилить свечение (`globalCompositeOperation: "lighter"`), проверить скриншотом на 1440.
